...
public class PubTaskUtil {
	private static Logger logger  =  Logger.getLogger(PubTaskUtil.class);
	
	public static final String I_ZHEJIANG_SYNC_KEY="i_zhejiang_sync";
	...
	private static final String AUTH_LOG_KEY = "TWIFI_AUTH_LOG";  //2.8适用
	private static final String TER_USER_AUTH_KEY = "TER_USER_AUTH";   //3.0中心平台接口定义的
	...
	private static Jedis GET_REDIS_HANDLER() {
		if (jedis == null ) {
			jedis = new Jedis(PropertiesUtil.confProperties.getProperty("center.task.memdb.ip"));			
		}
		else {
			if (!jedis.isConnected()) {
				PubTaskUtil.CLOSE_REDIS(jedis);
				jedis =  new Jedis(PropertiesUtil.confProperties.getProperty("center.task.memdb.ip"));
			}
		}
		return jedis;
	...
	public static String generateExecutableScript(String taskType, Map<String, String> params) throws IOException {
		...
		if (taskType.equalsIgnoreCase(TASK_TYPE_UPGRADE_COMPONENT)) {
			String componentType = params.get(PubTaskUtil.PARAM_UPGRADE_COMPONENT_TYPE);
			String scriptRelativePath = params.get(PubTaskUtil.PARAM_UPGRADE_SCRIPT_RELATIVE_PATH);
			...
			String packageBaseUrl = params.get(PubTaskUtil.PARAM_UPGRADE_PACKAGE_BASE_URL);
			
			String packageRelativeUrl = params.get(PubTaskUtil.PARAM_UPGRADE_PACKAGE_RELATIVE_URL);
	...
	* @param token
	*/
	public static void userAuthLogPush(String userMac, String authLogInfo) {
		Jedis jedis = GET_REDIS_HANDLER();
		try {
			long result = jedis.hset(AUTH_LOG_KEY, userMac, authLogInfo);
			logger.info("result: " + result + " TWIFI_AUTH_LOG set authLogInfo: " + authLogInfo);
		} finally {
			CLOSE_REDIS(jedis);
		}
	}
	...
	/**
	* 添加terminalUserAuthLog到redis
	*/
	public static void terminalUserAuthLogPush(String userMac, String authLogInfo) {
		Jedis jedis = GET_REDIS_HANDLER();
		try {
			long result = jedis.rpush(TER_USER_AUTH_KEY, authLogInfo);
			logger.info("result: " + result + " TER_USER_AUTH set authLogInfo: " + authLogInfo);
		} finally {
			CLOSE_REDIS(jedis);
		}
	}
	...
	* @return
	*/
	public static Map<String, Object> getUserAuthLog(String userMac) {
		Map<String, Object> authLogInfo = null;
		String strAuthLogInfo = StringUtils.EMPTY;
		if (StringUtils.isNotBlank(userMac)) {
			Jedis jedis = GET_REDIS_HANDLER();
			try {
				strAuthLogInfo = jedis.hget(AUTH_LOG_KEY, userMac);
			} finally {
				CLOSE_REDIS(jedis);
			}
		}
		if(StringUtils.isNotBlank(strAuthLogInfo)){
			authLogInfo = new Gson().fromJson(strAuthLogInfo, new TypeToken<Map<String, Object>>(){}.getType());
		}
		return authLogInfo;
	}
...
//              jedis.hset(STATIS_CACHE_KEY, key, new Gson().toJson(obj));
	...
	/**
	 * AC认证成功后  把认证信息写入redis
	 * i_zhejiang
	 * @param map
	 */
	public static void IZheJiangSyncAuthLog(String map) {
		Jedis jedis = GET_REDIS_HANDLER();
		try {
			long result = jedis.rpush(I_ZHEJIANG_SYNC_KEY, map);
			logger.info("result: " + result + " i_zhejiang_sync set authLogInfo: " + map);
		} finally {
			CLOSE_REDIS(jedis);
		}
	}
...
//       System.out.println(list.size());
    ...
    public static String getDefaultCallbackUrl() {
        return PubTaskUtil.assemblyDefaultPlatContextUrl(Constants.DEFAULT_CALLBACK_URL);
    }
    ...
    public static String getDefaultApiCallbackUrl() {
        return PubTaskUtil.assemblyDefaultPlatContextUrl(Constants.DEFAULT_API_CALLBACK_URL);
    }
    ...
    public static String getDefaultExpiredUrl() {
        return PubTaskUtil.assemblyDefaultPlatContextUrl(Constants.DEFAULT_EXPIRED_URL);
    }
    ...
    public static String getDefaultApiExpiredUrl() {
        return PubTaskUtil.assemblyDefaultPlatContextUrl(Constants.DEFAULT_API_EXPIRED_URL);
    }
...
/*	 Task format:
	...
	public static Map<String, Object> initialTask(String devId, String taskId, 
		...
		if (StringUtils.isNotBlank(devId) && StringUtils.isNotBlank(contextURL) && StringUtils.isNotBlank(taskType)) {
			...
			callbackObj.put("expired_url", contextURL + (StringUtils.isNotBlank(expiredUrl) ? expiredUrl : PubTaskUtil.getDefaultCallbackUrl()));
			callbackObj.put("callback_url", contextURL + (StringUtils.isNotBlank(callBackUrl) ? callBackUrl : PubTaskUtil.getDefaultExpiredUrl()));
			...
			String scriptString = PubTaskUtil.generateExecutableScript(taskType, taskPara);
    ...
    public static Map<String, Object> generateApiDeviceTaskData(String devId, long taskId, long taskCode,
        ...
        if (StringUtils.isNotBlank(devId)) {
			...
			Map<String, Object> callbackObj = new HashMap<String, Object>();
			callbackObj.put("expired_url", StringUtils.isNotBlank(expiredUrl) ? expiredUrl : PubTaskUtil.getDefaultApiExpiredUrl());
			callbackObj.put("callback_url", StringUtils.isNotBlank(callBackUrl) ? callBackUrl : PubTaskUtil.getDefaultApiCallbackUrl());
