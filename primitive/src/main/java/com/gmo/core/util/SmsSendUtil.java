...
public class SmsSendUtil {
	...
	static Logger logger = Logger.getLogger(new SmsSendUtil().getClass());
...
//	        System.out.println("===============================");
	 ...
	 * @throws IOException
	 */
	public static String sendSms3(String phoneNumber, String content, String sms_gateway,String type) throws IOException {
		String returnMessage = "";
		...
		if (StringUtils.isNotBlank(phoneNumber) && StringUtils.isNotBlank(content)) {
			...
			// 构建短信请求网络路径
			String strUrl = String.format(sms_gateway, phoneNumber, URLEncoder.encode(content, "UTF-8"),type);

			logger.error("请求短信网关：" + strUrl);
	        URL url = new URL(strUrl);
	        URLConnection conn = url.openConnection();
	        conn.setDoOutput(true);
	        BufferedReader br = new BufferedReader(new InputStreamReader(conn.getInputStream()));
